    ...
    */
    ...
    /*
     ...
     返回值 ： 0    插入数据成功
               -1   插入数据失败
    */
    function trace_tableI_insert(string producer_no, string vac_batch, string cdc_no, string trace_code, int state) public returns(int){
        int ret_code = 0;
        
		 if(order_check_select(producer_no, vac_batch, cdc_no) == 0)
		 {
			Table table = openTable(trace_code_table_name1);
    ...
    /*
     ...
     返回值 ： 0    删除数据成功
               -1   删除数据失败 
    */
    function trace_tableI_del(string producer_no, string vac_batch, string cdc_no) public returns(int){
        int ret_code = 0;
        
        TableFactory tf = TableFactory(0x1001);
        Table table = tf.openTable(trace_code_table_name1);
    ...
    /*
     描述 ：疾控中心修改溯源表I状态
     参数 ： 
			producer_no,vac_batch,cdc_no为主键，确定唯一数据
			producer_no  生产厂商编号
			vac_batch    疫苗批次
			cdc_no       疾控中心编号
			trace_code   溯源码
     ...
     返回值 ： 0    修改数据成功
               -1   修改数据失败 
    */
    function trace_tableI_change(string producer_no, string vac_batch, string cdc_no, string trace_code) public returns(int){
        int ret_code = 0;
        int state = 0;
	TableFactory tf = TableFactory(0x1001);
	Table table = tf.openTable(trace_code_table_name1);
    ...
    /*
     ...
     返回值 ： 
			...
			state[]       一致状态数组      
    */
    function trace_tableI_select(string producer_no, string vac_batch) constant public returns( bytes32[], bytes32[], int[]){
        Table table = openTable(trace_code_table_name1);
        Condition condition = table.newCondition();
        condition.EQ("vac_batch",vac_batch);
