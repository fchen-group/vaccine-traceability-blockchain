...
contract CdcTContract{
    ...
    /*
     ...
     返回值 ： 0    不存在 
               -1   已存在  
    */
    function cdc_no_select(string cdc_no) public returns(int){
        int ret_code = 0;
    ...
    /*
     ...
     参数 ： 
            ...
            name            疾控中心名称
            level           疾控中心级别
            receive_addr    疾控中心收货地址 
     返回值 ： 0    插入数据成功
               -1   插入数据失败
               -2   疾控中心编号已存在   
    */
    function cdc_insert(string cdc_no, address addr, string name, int level, string receive_addr) public returns(int){
        int ret_code = 0;
        
        // 如果疾控中心编号不存在
        if(cdc_no_select(cdc_no) == 0){
            cdc_num++;
            ...
            entry.set("cdc_name", name);
            entry.set("cdc_addr", int(addr));
            entry.set("cdc_level", level);
            entry.set("cdc_receive_addr", receive_addr);
    ...
    /*
     ...
     返回值 ： 0    删除数据成功
               -1   删除数据失败 
    */
    function cdc_del(string no) public returns(int){
        int ret_code = 0;
        
        TableFactory tf = TableFactory(0x1001);
        Table table = tf.openTable(cdc_table_name);
    ...
    /*
     ...
     返回值 ： 0    修改数据成功
               -1   修改数据失败 
    */
    function cdc_change(string no, address addr, string name, int level, string receive_addr) public returns(int){
        int ret_code = 0;
    ...
    /*
     ...
     返回值 ： 
            ...
            name            疾控中心名称
            level           疾控中心级别 
            recreceive_addr 疾控中心收货地址 
    */
    function cdc_select(string no) public returns(int ret_code, address addr, string name, int level, string receive_addr){
        Table table = openTable(cdc_table_name);
        Condition condition = table.newCondition();
        ...
        if(entries.size() == 0){
            ...
            addr = address(entry.getInt("cdc_addr"));
            name = byte32ToString(entry.getBytes32("cdc_name"));
            level = entry.getInt("cdc_level");
            receive_addr = byte32ToString(entry.getBytes32("cdc_receive_addr"));
        }
    }
}